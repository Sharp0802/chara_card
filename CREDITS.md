# Credits

`chara_card` would not be possible without the contributions and hard work of the following authors:

## Specifications

- [malfoyslastname/character-card-spec-v2](https://github.com/malfoyslastname/character-card-spec-v2)
  - **Files**: `docs/spec_v1.md`, `docs/spec_v2.md`
  - **Author**: [malfoyslastname](https://github.com/malfoyslastname)
  - **License**: No explicit license found.
  - **Copyright**: All rights reserved by the author.
  - **Note**: Included for reference and interoperability purposes.

- [kwaroran/character-card-spec-v3](https://github.com/kwaroran/character-card-spec-v3)
  - **Files**: `docs/spec_v3.md`
  - **Author**: [kwaroran](https://github.com/kwaroran)
  - **License**: MIT
  - **Copyright**: (c) 2024 kwaroran

## Dependencies

This project relies on the following cool open-source crates:

- [serde-rs/serde](https://github.com/serde-rs/serde) & [serde-rs/json](https://github.com/serde-rs/json)
  - **License**: Apache 2.0
  - **Copyright**: (c) 2015 Serde Contributors
  - **Note**: Used to handle serialization and deserialization.

- [BurntSushi/jiff](https://github.com/BurntSushi/jiff)
  - **License**: MIT
  - **Copyright**: (c) 2024 Andrew Gallant
  - **Note**: Used for date-time operations.
